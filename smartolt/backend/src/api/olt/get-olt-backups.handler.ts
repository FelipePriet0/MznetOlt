...
export async function getOltBackupsHandler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  try {
    const { data, error, count } = await supabase
      .from('olt_backups')
      .select('id, status, size_kb, next_backup_at, notes, backup_type, line_count, created_at', { count: 'exact' })
      .eq('olt_id', id)
